...
class TrainingSet:
    ...
    def createTrainingData(self, descriptor=Descriptor.TINY_GRAY4):
        ...
        self.trainData = np.ndarray(shape=(self.num_training_images,
                                           self.descriptor.getSize()),
                                    buffer=np.float32(np.array
                                                      (np.ravel(tempData))),
                                    dtype=np.float32)
        self.responses = np.ndarray(shape=(self.num_training_images, 1),
                                    buffer=np.float32(np.ravel(tempResponses)),
                                    dtype=np.float32)
        print('Training succeeded: computed descriptors for %d images' %
